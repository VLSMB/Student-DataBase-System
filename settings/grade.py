...
class Ui_grade(QtWidgets.QMainWindow):
    def reload(self):
        """重新读取"""
        gradedb = sql.getgrade()
        i = 0
        self.tbGrade.setRowCount(len(gradedb.keys()))
        for id, gd in gradedb.items():
            print(id, gd)
            self.tbGrade.setItem(i, 0, QtWidgets.QTableWidgetItem(str(id)))
            self.tbGrade.setItem(i, 1, QtWidgets.QTableWidgetItem(str(gd)))
            i += 1
        self.tbGrade.setEditTriggers(QtWidgets.QAbstractItemView.NoEditTriggers)
    def DELETE(self):
        if self.tbGrade.currentRow() == -1:
            QtWidgets.QMessageBox.warning(self, "提示", "没有选择要删除的项目！")
            return None
        id = self.tbGrade.item(self.tbGrade.currentRow(), 0).text()
        confirm = QtWidgets.QMessageBox.question(self, "提示", f"是否要删除年级ID {id} ？")
        if confirm == QtWidgets.QMessageBox.Yes:
            flag = sql.deltegrade(id)
            if flag:
                QtWidgets.QMessageBox.information(self, "提示", "删除年级成功！")
                self.reload()
            else:
    ...
    def ADD(self):
        ...
        if self.editID.text() == "" or self.editName.text() == "":
            ...
            flag = sql.addgrade(self.editID.text(), self.editName.text())
            if flag:
                QtWidgets.QMessageBox.information(self, "提示", "新年级增加成功！")
                self.reload()
            else:
                QtWidgets.QMessageBox.critical(self, "提示", "增加失败！")
    def EDIT(self):
        if self.tbGrade.currentColumn() == 0:
            text, flag = QtWidgets.QInputDialog.getInt(self, "修改ID", "请输入修改后的ID：", int(self.tbGrade.currentItem().text()), 0, 100, 1)
            if flag and text != "":
                sqlflag = sql.editgrade(0, text, self.tbGrade.item(self.tbGrade.currentRow(), 1).text())
                if sqlflag:
        ...
        elif self.tbGrade.currentColumn() == 1:
            text, flag = QtWidgets.QInputDialog.getText(self, "修改名称", "请输入修改之后的名称：", QtWidgets.QLineEdit.Normal, self.tbGrade.currentItem().text())
            if flag and text != "":
                sqlflag = sql.editgrade(1, self.tbGrade.item(self.tbGrade.currentRow(), 0).text(), text)
                if sqlflag:
                    QtWidgets.QMessageBox.information(self, "提示", "修改成功！")
                    self.reload()
                else:
    ...
    def setupUi(self, grade):
        ...
        self.btnExit.clicked.connect(grade.close)
    ...
    def retranslateUi(self, grade):
        ...
        grade.setWindowTitle(_translate("grade", "年级设置"))
        item = self.tbGrade.horizontalHeaderItem(0)
        item.setText(_translate("grade", "年级编号"))
        item = self.tbGrade.horizontalHeaderItem(1)
        item.setText(_translate("grade", "年级名称"))
        self.label_3.setText(_translate("grade", "年级名称："))
        self.label_2.setText(_translate("grade", "年级编号："))
        self.btnAdd.setText(_translate("grade", "添加"))
        self.btnEdit.setText(_translate("grade", "修改"))
        self.btnDel.setText(_translate("grade", "删除"))
        self.btnExit.setText(_translate("grade", "退出"))
import rc_rc
